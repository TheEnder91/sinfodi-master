...
@section('breadcrumb')
    <ol class="breadcrumb float-sm-right">
        ...
        <li class="breadcrumb-item active">Evaluaciones a la dirección administración->Acreditaciones</li>
    </ol>
@endsection

@section('content')
    @component('components.card')
        @slot('title_card', 'Evaluaciones a la dirección administración->Acreditaciones')
        <div class="row">
            <div class="col-3">
                        ...
                        <div class="tab-content" id="custom-tabs-one-tabContent">
                            @foreach ($criterios as $item)
                                <div class="tab-pane fade show {{ $item->id == 33 ? 'active' : '' }}" id="objetivos-{{ $item->id }}" role="tabpanel" aria-labelledby="objetivo-{{ $item->id }}">
                                    @if ($item->id == 33)
                                        @include('estimulos.evaluaciones.direccionAdministracion.acreditaciones.tblCriterio33')
                                    @endif
                                    @if ($item->id == 34)
                                        @include('estimulos.evaluaciones.direccionAdministracion.acreditaciones.tblCriterio34')
                                    @endif
                                    @if ($item->id == 35)
                                        @include('estimulos.evaluaciones.direccionAdministracion.acreditaciones.tblCriterio35')
                                    @endif
                                </div>
                            @endforeach
                        </div>
...
@section('scripts')
    <script>
        ...
        function initVer(){
            verDatos(0);
            $('#btnActualizarCriterio33').on('click', actualizarEvidenciasCriterio33);
            $('#btnActualizarCriterio34').on('click', actualizarEvidenciasCriterio34);
            $('#btnActualizarCriterio35').on('click', actualizarEvidenciasCriterio35);
        }
