...
@section('breadcrumb')
    <ol class="breadcrumb float-sm-right">
        ...
        <li class="breadcrumb-item active">Evaluaciones a la dirección de administración->Colaboracion institucional</li>
    </ol>
@endsection

@section('content')
    @component('components.card')
        @slot('title_card', 'Evaluación a la dirección administración->Colaboracion institucional')
        <div class="row">
            <div class="col-3">
...
@section('scripts')
    <script>
        ...
        function obtenerCriterio32(year){
            ...
            var criterio = 32;
            consultarDatos({
                action: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionAdministracion/colaboracion/searchColaboradores/" + año,
                type: 'GET',
                dataType: 'json',
                ok: function(datosCritero32){
                    ...
                    if(datosCriterio32.length > 0){
                        for(var i = 0; i < datosCriterio32.length; i++){
                            ...
                            $.ajax({
                                type: 'POST',
                                url: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionAdministracion/colaboracion/saveDatos",
                                data: {
        ...
        function verTablaCriterio32(year, criterio) {
            consultarDatos({
                action: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionAdministracion/colaboracion/datosColaboradores/" + year + "/" + criterio,
                type: 'GET',
                dataType: 'json',
                ok: function(datosAdministracionCriterio32){
                    // console.log(datosAdministracionCriterio32);
                    var datosAdministracionCriterio32 = datosAdministracionCriterio32.response;
                    var row = "";
                    for(var i = 0; i < datosAdministracionCriterio32.length; i++){
                        var dataAdministracionCriterio32 = datosAdministracionCriterio32[i];
                        // console.log(dataAdministracionCriterio32);
                        var authUser = '<?= Auth::user()->usuario ?>';
                        var permissions = '<?= Auth::user()->hasPermissionTo("estimulo-evaluaciones-administracion-colaboracion-index") ?>';
                        // console.log(permissions);
                        if(dataAdministracionCriterio32.username == authUser || permissions == 1){
                                row += "<tr>";
                                row += '<th scope="row" class="text-center" width="8%" style="font-size:12px;">' + dataAdministracionCriterio32.clave + '</td>';
                                row += '<td width="77%" style="font-size:12px;">' + dataAdministracionCriterio32.nombre.toUpperCase() + "</td>";
                                row += '<td class="text-center" width="5%" style="font-size:12px;">' + Math.trunc(dataAdministracionCriterio32.puntos) + '</td>';
                                row += '<td class="text-center" width="5%" style="font-size:12px;">' + Math.trunc(dataAdministracionCriterio32.total_puntos) + '</td>';
                                row += '<td class="text-center" width="5%" style="font-size:12px;">' + dataAdministracionCriterio32.year + '</td>';
                                row += '<td class="text-center" width="5%" style="font-size:12px;"><a href="javascript:verDetalleCriterio32(' + dataAdministracionCriterio32.year + ', ' + dataAdministracionCriterio32.clave + ')"><i class="fa fa-search"></i></a></td>';
                                row += "</tr>";
                        }
                    }
        ...
        function verDetalleCriterio32(year, clave){
            ...
            var puntos = 10;
            consultarDatos({
                action: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionAdministracion/colaboracion/getComiteColaboracion/" + year + "/" + clave,
                type: 'GET',
                dataType: 'json',
