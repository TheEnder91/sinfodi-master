<div class="table-responsive" width = "100%" id="table_refresh">
    <table id="tblCriterio28" class="table table-bordered table-striped" style="font-size:13px;">
        <caption style="font-size:13px;">Atención a alumnos de tesis de técnico superior universitario o equivalente (Valor del punto: 10).</caption>
        <thead>
...
@include('estimulos.evaluaciones.direccionAdministracion.formacion.modalEvidenciasCriterio28')
...
<script>
            ...
            ok: function(datosCritero28){
                ...
                if(datosCriterio28.length > 0){
                    for(var i = 0; i < datosCriterio28.length; i++){
                        ...
                        consultarDatos({
                            action: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionAdministracion/formacionRH/searchUsernameFormacionRH/" + dataCriterio28.numero_personal,
                            type: 'GET',
                            dataType: 'json',
    ...
    function verTablaCriterio28(year, criterio){
        consultarDatos({
            ...
            type: 'GET',
            dataType: 'json',
            ok: function(datosAdministracionCriterio28){
                // console.log(datosAdministracionCriterio28);
                var datosAdministracionCriterio28 = datosAdministracionCriterio28.response;
                var row = "";
                for(var i = 0; i < datosAdministracionCriterio28.length; i++){
                    var dataAdministracionCriterio28 = datosAdministracionCriterio28[i];
                    // console.log(dataAdministracionCriterio28);
                    var authUser = '<?= Auth::user()->usuario ?>';
                    var permissions = '<?= Auth::user()->hasPermissionTo("estimulo-evaluaciones-administracion-formacion-index") ?>';
                    // console.log(permissions);
                    if(dataAdministracionCriterio28.username == authUser || permissions == 1){
                        row += "<tr>";
                        row += '<th scope="row" class="text-center" width="10%" style="font-size:12px;">' + dataAdministracionCriterio28.clave + '</td>';
                        row += '<td width="40%" style="font-size:12px;">' + dataAdministracionCriterio28.nombre.toUpperCase() + "</td>";
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + parseInt(dataAdministracionCriterio28.puntos) + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + parseInt(dataAdministracionCriterio28.total_puntos) + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + dataAdministracionCriterio28.year + '</td>';
                        // if(permissions == 1){
                            row += '<td class="text-center" width="10%" style="font-size:12px;"><a href="javascript:verEvidenciasCriterio28(' + dataAdministracionCriterio28.year + ', ' + dataAdministracionCriterio28.clave + ', ' + 28 +')"><i class="fa fa-edit"></i></a></td>';
                        // }
                        row += "</tr>";
                    }
                }
                    ...
                    ok: function(puntosCriterio28){
                        ...
                        for(var i = 0; i < datos.length; i++){
                            ...
                            if(claveData.evidencias != null){
                                ...
                                row += '<br>';
                                row += '<b><input type="checkbox" class="evidenciasCriterio28" name="evidenciasCriterio28[]" id="evidenciasCriterio28'+claveEvidencias+'" value="'+claveEvidencias+'" onClick="contarEvidenciasCriterio28('+puntos+');"> ' + claveEvidencias + ' -> ' + claveData.porcentaje + '%</b>';
                                row += '</div>';
                            }
                        }
