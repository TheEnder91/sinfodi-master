...
@include('estimulos.evaluaciones.direccionAdministracion.formacion.modalEvidenciasCriterio30')
...
<script>
            ...
            ok: function(datosCritero30){
                ...
                if(datosCriterio30.length > 0){
                    for(var i = 0; i < datosCriterio30.length; i++){
                        ...
                        consultarDatos({
                            action: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionAdministracion/formacionRH/searchUsernameFormacionRH/" + dataCriterio30.numero_personal,
                            type: 'GET',
                            dataType: 'json',
    ...
    function verTablaCriterio30(year, criterio){
        consultarDatos({
            ...
            type: 'GET',
            dataType: 'json',
            ok: function(datosAdministracionCriterio30){
                // console.log(datosAdministracionCriterio30);
                var datosAdministracionCriterio30 = datosAdministracionCriterio30.response;
                var row = "";
                for(var i = 0; i < datosAdministracionCriterio30.length; i++){
                    var dataAdministracionCriterio30 = datosAdministracionCriterio30[i];
                    // console.log(dataAdministracionCriterio30);
                    var authUser = '<?= Auth::user()->usuario ?>';
                    var permissions = '<?= Auth::user()->hasPermissionTo("estimulo-evaluaciones-administracion-formacion-index") ?>';
                    // console.log(permissions);
                    if(dataAdministracionCriterio30.username == authUser || permissions == 1){
                        row += "<tr>";
                        row += '<th scope="row" class="text-center" width="10%" style="font-size:12px;">' + dataAdministracionCriterio30.clave + '</td>';
                        row += '<td width="40%" style="font-size:12px;">' + dataAdministracionCriterio30.nombre.toUpperCase() + "</td>";
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + parseInt(dataAdministracionCriterio30.puntos) + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + parseInt(dataAdministracionCriterio30.total_puntos) + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + dataAdministracionCriterio30.year + '</td>';
                        // if(permissions == 1){
                            row += '<td class="text-center" width="10%" style="font-size:12px;"><a href="javascript:verEvidenciasCriterio30(' + dataAdministracionCriterio30.year + ', ' + dataAdministracionCriterio30.clave + ', ' + 30 +')"><i class="fa fa-edit"></i></a></td>';
                        // }
                        row += "</tr>";
                    }
                }
                    ...
                    ok: function(puntosCriterio30){
                        ...
                        for(var i = 0; i < datos.length; i++){
                            ...
                            if(claveData.evidencias != null){
                                ...
                                row += '<br>';
                                row += '<b><input type="checkbox" class="evidenciasCriterio30" name="evidenciasCriterio30[]" id="evidenciasCriterio30'+claveEvidencias+'" value="'+claveEvidencias+'" onClick="contarEvidenciasCriterio30('+puntos+');"> ' + claveEvidencias + '</b>';
                                row += '</div>';
                            }
                        }
