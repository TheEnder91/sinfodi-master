<div class="table-responsive" width = "100%" id="table_refresh">
    <table id="tblCriterio19" class="table table-bordered table-striped" style="font-size:13px;">
        <caption style="font-size:13px;">Registro de diseño industrial otorgado.</caption>
        <thead>
...
@include('estimulos.evaluaciones.direccionAdministracion.transferencia.modalEvidenciasCriterio19')
...
<script>
    ...
    function verTablaCriterio19(year, criterio){
        consultarDatos({
            ...
            type: 'GET',
            dataType: 'json',
            ok: function(datosAdministracionCriterio19){
                // console.log(datosAdministracionCriterio19);
                var datosAdministracionCriterio19 = datosAdministracionCriterio19.response;
                var row = "";
                for(var i = 0; i < datosAdministracionCriterio19.length; i++){
                    var dataAdministracionCriterio19 = datosAdministracionCriterio19[i];
                    // console.log(dataAdministracionCriterio19);
                    var authUser = '<?= Auth::user()->usuario ?>';
                    var permissions = '<?= Auth::user()->hasPermissionTo("estimulo-evaluaciones-administracion-transferencia-index") ?>';
                    // console.log(permissions);
                    if(dataAdministracionCriterio19.username == authUser || permissions == 1){
                        row += "<tr>";
                        row += '<th scope="row" class="text-center" width="10%" style="font-size:12px;">' + dataAdministracionCriterio19.clave + '</td>';
                        row += '<td width="40%" style="font-size:12px;">' + dataAdministracionCriterio19.nombre.toUpperCase() + "</td>";
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + parseInt(dataAdministracionCriterio19.puntos) + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + parseInt(dataAdministracionCriterio19.total_puntos) + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + dataAdministracionCriterio19.year + '</td>';
                        if(permissions == 1){
                            row += '<td class="text-center" width="10%" style="font-size:12px;"><a href="javascript:verEvidenciasCriterio19(' + dataAdministracionCriterio19.year + ', ' + dataAdministracionCriterio19.clave + ', ' + 19 +')"><i class="fa fa-edit"></i></a></td>';
                        }
                        row += "</tr>";
                    }
                }
                ...
                consultarDatos({
                    ...
                    ok: function(puntosCriterio19){
                        ...
                        for(var i = 0; i < datos.length; i++){
                            ...
                            row += '</a><br>';
                            row += '<b><input type="checkbox" class="evidenciasCriterio19" name="evidenciasCriterio19[]" id="evidenciasCriterio19'+claveData.clave+'" value="'+claveData.clave+'" onClick="contarEvidencias('+puntos+');"> ' + claveData.clave + tipo + '</b>';
                            row += '</div>';
                        }
