...
@include('estimulos.evaluaciones.direccionAdministracion.transferencia.modalEvidenciasCriterio21')
...
<script>
    ...
    function verTablaCriterio21(year, criterio){
        consultarDatos({
            ...
            type: 'GET',
            dataType: 'json',
            ok: function(datosAdministracionCriterio21){
                // console.log(datosAdministracionCriterio21);
                var datosAdministracionCriterio21 = datosAdministracionCriterio21.response;
                var row = "";
                for(var i = 0; i < datosAdministracionCriterio21.length; i++){
                    var dataAdministracionCriterio21 = datosAdministracionCriterio21[i];
                    // console.log(dataAdministracionCriterio21);
                    var authUser = '<?= Auth::user()->usuario ?>';
                    var permissions = '<?= Auth::user()->hasPermissionTo("estimulo-evaluaciones-administracion-transferencia-index") ?>';
                    // console.log(permissions);
                    if(dataAdministracionCriterio21.username == authUser || permissions == 1){
                        row += "<tr>";
                        row += '<th scope="row" class="text-center" width="10%" style="font-size:12px;">' + dataAdministracionCriterio21.clave + '</td>';
                        row += '<td width="40%" style="font-size:12px;">' + dataAdministracionCriterio21.nombre.toUpperCase() + "</td>";
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + parseInt(dataAdministracionCriterio21.puntos) + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + parseInt(dataAdministracionCriterio21.total_puntos) + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + dataAdministracionCriterio21.year + '</td>';
                        // if(permissions == 1){
                            row += '<td class="text-center" width="10%" style="font-size:12px;"><a href="javascript:verEvidenciasCriterio21(' + dataAdministracionCriterio21.year + ', ' + dataAdministracionCriterio21.clave + ', ' + 21 +')"><i class="fa fa-edit"></i></a></td>';
                        // }
                        row += "</tr>";
                    }
                }
                ...
                consultarDatos({
                    ...
                    ok: function(puntosCriterio21){
                        ...
                        for(var i = 0; i < datos.length; i++){
                            ...
                            row += '</a><br>';
                            row += '<b><input type="checkbox" class="evidenciasCriterio21" name="evidenciasCriterio21[]" id="evidenciasCriterio21'+claveData.clave+'" value="'+claveData.clave+'" onClick="contarEvidencias('+puntos+');"> ' + claveData.clave + tipo + '</b>';
                            row += '</div>';
                        }
