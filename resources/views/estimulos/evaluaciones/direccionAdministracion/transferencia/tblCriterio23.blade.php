<div class="table-responsive" width = "100%" id="table_refresh">
    <table id="tblCriterio23" class="table table-bordered table-striped" style="font-size:13px;">
        <caption style="font-size:13px;">Formalización de contrato de transferencia.</caption>
        <thead>
...
@include('estimulos.evaluaciones.direccionAdministracion.transferencia.modalEvidenciasCriterio23')
...
<script>
    ...
    function verTablaCriterio23(year, criterio){
        consultarDatos({
            ...
            type: 'GET',
            dataType: 'json',
            ok: function(datosAdministracionCriterio23){
                // console.log(datosAdministracionCriterio23);
                var datosAdministracionCriterio23 = datosAdministracionCriterio23.response;
                var row = "";
                for(var i = 0; i < datosAdministracionCriterio23.length; i++){
                    var dataAdministracionCriterio23 = datosAdministracionCriterio23[i];
                    // console.log(dataAdministracionCriterio23);
                    var authUser = '<?= Auth::user()->usuario ?>';
                    var permissions = '<?= Auth::user()->hasPermissionTo("estimulo-evaluaciones-administracion-transferencia-index") ?>';
                    // console.log(permissions);
                    if(dataAdministracionCriterio23.username == authUser || permissions == 1){
                        row += "<tr>";
                        row += '<th scope="row" class="text-center" width="10%" style="font-size:12px;">' + dataAdministracionCriterio23.clave + '</td>';
                        row += '<td width="40%" style="font-size:12px;">' + dataAdministracionCriterio23.nombre.toUpperCase() + "</td>";
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + parseInt(dataAdministracionCriterio23.puntos) + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + parseInt(dataAdministracionCriterio23.total_puntos) + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + dataAdministracionCriterio23.year + '</td>';
                        if(permissions == 1){
                            row += '<td class="text-center" width="10%" style="font-size:12px;"><a href="javascript:verEvidenciasCriterio23(' + dataAdministracionCriterio23.year + ', ' + dataAdministracionCriterio23.clave + ', ' + 23 +')"><i class="fa fa-edit"></i></a></td>';
                        }
                        row += "</tr>";
                    }
                }
                ...
                consultarDatos({
                    ...
                    ok: function(puntosCriterio23){
                        ...
                        for(var i = 0; i < datos.length; i++){
                            ...
                            row += '</a><br>';
                            row += '<b><input type="checkbox" class="evidenciasCriterio23" name="evidenciasCriterio23[]" id="evidenciasCriterio23'+claveData.clave+'" value="'+claveData.clave+'" onClick="contarEvidencias('+puntos+');"> ' + claveData.clave + tipo + '</b>';
                            row += '</div>';
                        }
