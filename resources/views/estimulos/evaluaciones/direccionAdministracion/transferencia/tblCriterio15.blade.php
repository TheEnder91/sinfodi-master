<div class="table-responsive" width = "100%" id="table_refresh">
    <table id="tblCriterio15" class="table table-bordered table-striped" style="font-size:13px;">
        <caption style="font-size:13px;">Solicitud de patente.</caption>
        <thead>
...
@include('estimulos.evaluaciones.direccionAdministracion.transferencia.modalEvidenciasCriterio15')
...
<script>
    ...
    function verTablaCriterio15(year, criterio){
        consultarDatos({
            ...
            type: 'GET',
            dataType: 'json',
            ok: function(datosAdministracionCriterio15){
                // console.log(datosAdministracionCriterio15);
                var datosAdministracionCriterio15 = datosAdministracionCriterio15.response;
                var row = "";
                for(var i = 0; i < datosAdministracionCriterio15.length; i++){
                    var dataAdministracionCriterio15 = datosAdministracionCriterio15[i];
                    // console.log(dataAdministracionCriterio15);
                    var authUser = '<?= Auth::user()->usuario ?>';
                    var permissions = '<?= Auth::user()->hasPermissionTo("estimulo-evaluaciones-administracion-transferencia-index") ?>';
                    // console.log(permissions);
                    if(dataAdministracionCriterio15.username == authUser || permissions == 1){
                        row += "<tr>";
                        row += '<th scope="row" class="text-center" width="10%" style="font-size:12px;">' + dataAdministracionCriterio15.clave + '</td>';
                        row += '<td width="40%" style="font-size:12px;">' + dataAdministracionCriterio15.nombre.toUpperCase() + "</td>";
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + parseInt(dataAdministracionCriterio15.puntos) + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + parseInt(dataAdministracionCriterio15.total_puntos) + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + dataAdministracionCriterio15.year + '</td>';
                        if(permissions == 1){
                            row += '<td class="text-center" width="10%" style="font-size:12px;"><a href="javascript:verEvidenciasCriterio15(' + dataAdministracionCriterio15.year + ', ' + dataAdministracionCriterio15.clave + ', ' + 15 +')"><i class="fa fa-edit"></i></a></td>';
                        }
                        row += "</tr>";
                    }
                }
                ...
                consultarDatos({
                    ...
                    ok: function(puntosCriterio15){
                        ...
                        for(var i = 0; i < datos.length; i++){
                            ...
                            row += '</a><br>';
                            row += '<b><input type="checkbox" class="evidenciasCriterio15" name="evidenciasCriterio15[]" id="evidenciasCriterio15'+claveData.clave+'" value="'+claveData.clave+'" onClick="contarEvidencias('+puntos+');"> ' + claveData.clave + tipo + '</b>';
                            row += '</div>';
                        }
