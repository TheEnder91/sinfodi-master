...
@section('breadcrumb')
    <ol class="breadcrumb float-sm-right">
        ...
        <li class="breadcrumb-item active">Evaluaciones a la dirección de tecnologia->Colaboracion institucional</li>
    </ol>
@endsection

@section('content')
    @component('components.card')
        @slot('title_card', 'Evaluación a la dirección de tecnologia->Colaboracion institucional')
        <div class="row">
            <div class="col-3">
...
@section('scripts')
    <script>
        ...
        function obtenerCriterio32(year){
            ...
            var criterio = 32;
            consultarDatos({
                action: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionProyTec/colaboracion/searchColaboradores/" + año,
                type: 'GET',
                dataType: 'json',
        ...
        function verTablaCriterio32(year, criterio) {
            consultarDatos({
                action: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionProyTec/colaboracion/datosColaboradores/" + year + "/" + criterio,
                type: 'GET',
                dataType: 'json',
                ok: function(datosProyectosCriterio32){
                    // console.log(datosProyectosCriterio32);
                    var datosProyectosCriterio32 = datosProyectosCriterio32.response;
                    var row = "";
                    for(var i = 0; i < datosProyectosCriterio32.length; i++){
                        var dataProyectosCriterio32 = datosProyectosCriterio32[i];
                        // console.log(dataProyectosCriterio32);
                        var authUser = '<?= Auth::user()->usuario ?>';
                        var permissions = '<?= Auth::user()->hasPermissionTo("estimulo-evaluaciones-proyectos-colaboracion-index") ?>';
                        // console.log(permissions);
                        if(dataProyectosCriterio32.username == authUser || permissions == 1){
                                row += "<tr>";
                                row += '<th scope="row" class="text-center" width="8%" style="font-size:12px;">' + dataProyectosCriterio32.clave + '</td>';
                                row += '<td width="77%" style="font-size:12px;">' + dataProyectosCriterio32.nombre.toUpperCase() + "</td>";
                                row += '<td class="text-center" width="5%" style="font-size:12px;">' + Math.trunc(dataProyectosCriterio32.puntos) + '</td>';
                                row += '<td class="text-center" width="5%" style="font-size:12px;">' + Math.trunc(dataProyectosCriterio32.total_puntos) + '</td>';
                                row += '<td class="text-center" width="5%" style="font-size:12px;">' + dataProyectosCriterio32.year + '</td>';
                                row += '<td class="text-center" width="5%" style="font-size:12px;"><a href="javascript:verDetalleCriterio32(' + dataProyectosCriterio32.year + ', ' + dataProyectosCriterio32.clave + ')"><i class="fa fa-search"></i></a></td>';
                                row += "</tr>";
                        }
                    }
        ...
        function verDetalleCriterio32(year, clave){
            ...
            var puntos = 10;
            consultarDatos({
                action: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionProyTec/colaboracion/getComiteColaboracion/" + year + "/" + clave,
                type: 'GET',
                dataType: 'json',
