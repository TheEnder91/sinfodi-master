<div class="table-responsive" width = "100%" id="table_refresh">
    <table id="tblCriterio38" class="table table-bordered table-striped" style="font-size:13px;">
        <caption style="font-size:13px;">Proyectos interinstitucionales.</caption>
        <thead>
...
<script>
    ...
    function verTablaCriterio38(year, criterio){
        consultarDatos({
            ...
            type: 'GET',
            dataType: 'json',
            ok: function(datosProyectosCriterio38){
                var datosProyectosCriterio38 = datosProyectosCriterio38.response;
                // console.log(datosProyectosCriterio38);
                var row = "";
                for(var i = 0; i < datosProyectosCriterio38.length; i++){
                    var dataProyectosCriteri38 = datosProyectosCriterio38[i];
                    // console.log(dataProyectosCriteri38);
                    var authUser = '<?= Auth::user()->usuario ?>';
                    var permissions = '<?= Auth::user()->hasPermissionTo("estimulo-evaluaciones-proyectos-transferenciaB-index") ?>';
                    // console.log(permissions);
                    if(dataProyectosCriteri38.username == authUser || permissions == 1){
                            row += "<tr>";
                            row += '<th style="font-size:12px;" scope="row" class="text-center" width="10%">' + dataProyectosCriteri38.clave + '</td>';
                            row += '<td style="font-size:12px;" width="60%">' + dataProyectosCriteri38.nombre.toUpperCase() + "</td>";
                            row += '<td style="font-size:12px;" class="text-center" width="10%">' + parseInt(dataProyectosCriteri38.puntos) + '</td>';
                            row += '<td style="font-size:12px;" class="text-center" width="10%">' + parseInt(dataProyectosCriteri38.total_puntos) + '</td>';
                            row += '<td style="font-size:12px;" class="text-center" width="10%">' + dataProyectosCriteri38.year + '</td>';
                            row += "</tr>";
                    }
                }
