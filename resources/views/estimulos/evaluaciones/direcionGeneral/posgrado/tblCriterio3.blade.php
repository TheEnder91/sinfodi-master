...
@include('estimulos.evaluaciones.direcionGeneral.posgrado.modalEvidenciasCriterio3')
...
<script>
            ...
            ok: function(datosCritero3){
                ...
                if(datosCriterio3.length > 0){
                    for(var i = 0; i < datosCriterio3.length; i++){
                        ...
                        consultarDatos({
                            action: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionGeneral/posgrado/searchUsernamePosgrado/" + dataCriterio3.numero_personal,
                            type: 'GET',
                            dataType: 'json',
    ...
    function verTablaCriterio3(year, criterio){
        consultarDatos({
            ...
            type: 'GET',
            dataType: 'json',
            ok: function(datosGeneralCriterio3){
                // console.log(datosGeneralCriterio3);
                var datosGeneralCriterio3 = datosGeneralCriterio3.response;
                var row = "";
                for(var i = 0; i < datosGeneralCriterio3.length; i++){
                    var dataGeneralCriterio3 = datosGeneralCriterio3[i];
                    // console.log(dataGeneralCriterio3);
                    var authUser = '<?= Auth::user()->usuario ?>';
                    var permissions = '<?= Auth::user()->hasPermissionTo("estimulo-evaluaciones-direccionGral-posgrado-index") ?>';
                    // console.log(permissions);
                    if(dataGeneralCriterio3.username == authUser || permissions == 1){
                        row += "<tr>";
                        row += '<th scope="row" class="text-center" width="10%" style="font-size:12px;">' + dataGeneralCriterio3.clave + '</td>';
                        row += '<td width="40%" style="font-size:12px;">' + dataGeneralCriterio3.nombre.toUpperCase() + "</td>";
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + parseInt(dataGeneralCriterio3.puntos) + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + parseInt(dataGeneralCriterio3.total_puntos) + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + dataGeneralCriterio3.year + '</td>';
                        if(permissions == 1){
                            row += '<td class="text-center" width="10%" style="font-size:12px;"><a href="javascript:verEvidenciasCriterio3(' + dataGeneralCriterio3.year + ', ' + dataGeneralCriterio3.clave + ', ' + criterio +')"><i class="fa fa-edit"></i></a></td>';
                        }
                        row += "</tr>";
                    }
                }
