                        ...
                        <div class="tab-content" id="custom-tabs-one-tabContent">
                            @foreach ($criterios as $item)
                                <div class="tab-pane fade show {{ $item->id == 38 ? 'active' : '' }}" id="objetivos-{{ $item->id }}" role="tabpanel" aria-labelledby="objetivo-{{ $item->id }}">
                                    @if ($item->id == 38)
                                        @include('estimulos.evaluaciones.direcionGeneral.transferenciaB.tblCriterio38')
                                    @endif
                                    @if ($item->id == 39)
                                        @include('estimulos.evaluaciones.direcionGeneral.transferenciaB.tblCriterio39')
                                    @endif
                                    @if ($item->id == 40)
                                        @include('estimulos.evaluaciones.direcionGeneral.transferenciaB.tblCriterio40')
                                    @endif
                                    @if ($item->id == 41)
                                        @include('estimulos.evaluaciones.direcionGeneral.transferenciaB.tblCriterio41')
                                    @endif
                                </div>
                            @endforeach
                        </div>
