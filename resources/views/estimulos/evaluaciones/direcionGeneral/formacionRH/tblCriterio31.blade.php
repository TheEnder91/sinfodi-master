<div class="table-responsive" width = "100%" id="table_refresh">
    <table id="tblCriterio31" class="table table-bordered table-striped" style="font-size:13px;">
        <caption style="font-size:13px;">Atención a alumnos de tesis de doctorado concluida (Valor del punto: 40).</caption>
        <thead>
...
@include('estimulos.evaluaciones.direcionGeneral.formacionRH.modalEvidenciasCriterio31')
...
<script>
            ...
            ok: function(datosCritero31){
                ...
                if(datosCriterio31.length > 0){
                    for(var i = 0; i < datosCriterio31.length; i++){
                        ...
                        consultarDatos({
                            action: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionGeneral/formacionRH/searchUsernameFormacionRH/" + dataCriterio31.numero_personal,
                            type: 'GET',
                            dataType: 'json',
    ...
    function verTablaCriterio31(year, criterio){
        consultarDatos({
            ...
            type: 'GET',
            dataType: 'json',
            ok: function(datosGeneralCriterio31){
                // console.log(datosGeneralCriterio31);
                var datosGeneralCriterio31 = datosGeneralCriterio31.response;
                var row = "";
                for(var i = 0; i < datosGeneralCriterio31.length; i++){
                    var dataGeneralCriterio31 = datosGeneralCriterio31[i];
                    // console.log(dataGeneralCriterio31);
                    var authUser = '<?= Auth::user()->usuario ?>';
                    var permissions = '<?= Auth::user()->hasPermissionTo("estimulo-evaluaciones-general-transferencia-index") ?>';
                    // console.log(permissions);
                    if(dataGeneralCriterio31.username == authUser || permissions == 1){
                        row += "<tr>";
                        row += '<th scope="row" class="text-center" width="10%" style="font-size:12px;">' + dataGeneralCriterio31.clave + '</td>';
                        row += '<td width="40%" style="font-size:12px;">' + dataGeneralCriterio31.nombre.toUpperCase() + "</td>";
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + parseInt(dataGeneralCriterio31.puntos) + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + parseInt(dataGeneralCriterio31.total_puntos) + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + dataGeneralCriterio31.year + '</td>';
                        // if(permissions == 1){
                            row += '<td class="text-center" width="10%" style="font-size:12px;"><a href="javascript:verEvidenciasCriterio31(' + dataGeneralCriterio31.year + ', ' + dataGeneralCriterio31.clave + ', ' + 31 +')"><i class="fa fa-edit"></i></a></td>';
                        // }
                        row += "</tr>";
                    }
                }
