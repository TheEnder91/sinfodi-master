                        ...
                        <div class="tab-content" id="custom-tabs-one-tabContent">
                            @foreach ($criterios as $item)
                                <div class="tab-pane fade show {{ $item->id == 24 ? 'active' : '' }}" id="objetivos-{{ $item->id }}" role="tabpanel" aria-labelledby="objetivo-{{ $item->id }}">
                                    @if ($item->id == 24)
                                        @include('estimulos.evaluaciones.direcionGeneral.formacionRH.tblCriterio24')
                                    @endif
                                    @if ($item->id == 25)
                                        @include('estimulos.evaluaciones.direcionGeneral.formacionRH.tblCriterio25')
                                    @endif
                                    @if ($item->id == 26)
                                        @include('estimulos.evaluaciones.direcionGeneral.formacionRH.tblCriterio26')
                                    @endif
                                    @if ($item->id == 27)
                                        @include('estimulos.evaluaciones.direcionGeneral.formacionRH.tblCriterio27')
                                    @endif
                                    @if ($item->id == 28)
                                        @include('estimulos.evaluaciones.direcionGeneral.formacionRH.tblCriterio28')
                                    @endif
                                    @if ($item->id == 29)
                                        @include('estimulos.evaluaciones.direcionGeneral.formacionRH.tblCriterio29')
                                    @endif
                                    @if ($item->id == 30)
                                        @include('estimulos.evaluaciones.direcionGeneral.formacionRH.tblCriterio30')
                                    @endif
                                    @if ($item->id == 31)
                                        @include('estimulos.evaluaciones.direcionGeneral.formacionRH.tblCriterio31')
                                    @endif
                                </div>
                            @endforeach
                        </div>
