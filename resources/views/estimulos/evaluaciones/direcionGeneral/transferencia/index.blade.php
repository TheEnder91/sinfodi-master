                        ...
                        <div class="tab-content" id="custom-tabs-one-tabContent">
                            @foreach ($criterios as $item)
                                <div class="tab-pane fade show {{ $item->id == 15 ? 'active' : '' }}" id="objetivos-{{ $item->id }}" role="tabpanel" aria-labelledby="objetivo-{{ $item->id }}">
                                    @if ($item->id == 15)
                                        @include('estimulos.evaluaciones.direcionGeneral.transferencia.tblCriterio15')
                                    @endif
                                    @if ($item->id == 16)
                                        @include('estimulos.evaluaciones.direcionGeneral.transferencia.tblCriterio16')
                                    @endif
                                    @if ($item->id == 17)
                                        @include('estimulos.evaluaciones.direcionGeneral.transferencia.tblCriterio17')
                                    @endif
                                    @if ($item->id == 18)
                                        @include('estimulos.evaluaciones.direcionGeneral.transferencia.tblCriterio18')
                                    @endif
                                    @if ($item->id == 19)
                                        @include('estimulos.evaluaciones.direcionGeneral.transferencia.tblCriterio19')
                                    @endif
                                    @if ($item->id == 20)
                                        @include('estimulos.evaluaciones.direcionGeneral.transferencia.tblCriterio20')
                                    @endif
                                    @if ($item->id == 21)
                                        @include('estimulos.evaluaciones.direcionGeneral.transferencia.tblCriterio21')
                                    @endif
                                    @if ($item->id == 22)
                                        @include('estimulos.evaluaciones.direcionGeneral.transferencia.tblCriterio22')
                                    @endif
                                    @if ($item->id == 23)
                                        @include('estimulos.evaluaciones.direcionGeneral.transferencia.tblCriterio23')
                                    @endif
                                </div>
                            @endforeach
                        </div>
