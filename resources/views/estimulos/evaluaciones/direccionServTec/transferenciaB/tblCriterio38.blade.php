...
<script>
    ...
    function verTablaCriterio38(year, criterio){
        consultarDatos({
            ...
            type: 'GET',
            dataType: 'json',
            ok: function(datosServiciosCriterio38){
                var datosServiciosCriterio38 = datosServiciosCriterio38.response;
                // console.log(datosServiciosCriterio38);
                var row = "";
                for(var i = 0; i < datosServiciosCriterio38.length; i++){
                    var dataServiciosCriteri38 = datosServiciosCriterio38[i];
                    // console.log(dataServiciosCriteri38);
                    var authUser = '<?= Auth::user()->usuario ?>';
                    var permissions = '<?= Auth::user()->hasPermissionTo("estimulo-evaluaciones-servicios-transferenciaB-index") ?>';
                    // console.log(permissions);
                    if(dataServiciosCriteri38.username == authUser || permissions == 1){
                            row += "<tr>";
                            row += '<th style="font-size:12px;" scope="row" class="text-center" width="10%">' + dataServiciosCriteri38.clave + '</td>';
                            row += '<td style="font-size:12px;" width="60%">' + dataServiciosCriteri38.nombre.toUpperCase() + "</td>";
                            row += '<td style="font-size:12px;" class="text-center" width="10%">' + parseInt(dataServiciosCriteri38.puntos) + '</td>';
                            row += '<td style="font-size:12px;" class="text-center" width="10%">' + parseInt(dataServiciosCriteri38.total_puntos) + '</td>';
                            row += '<td style="font-size:12px;" class="text-center" width="10%">' + dataServiciosCriteri38.year + '</td>';
                            row += "</tr>";
                    }
                }
