...
@section('breadcrumb')
    <ol class="breadcrumb float-sm-right">
        ...
        <li class="breadcrumb-item active">Evaluaciones a la dirección de servicios tecnologicos->Transferencia de conocimiento B</li>
    </ol>
@endsection

@section('content')
    @component('components.card')
        @slot('title_card', 'Evaluaciones a la dirección de servicios tecnologicos->Transferencia de conocimiento B')
        <div class="row">
            <div class="col-3">
                        ...
                        <div class="tab-content" id="custom-tabs-one-tabContent">
                            @foreach ($criterios as $item)
                                <div class="tab-pane fade show {{ $item->id == 38 ? 'active' : '' }}" id="objetivos-{{ $item->id }}" role="tabpanel" aria-labelledby="objetivo-{{ $item->id }}">
                                    @if ($item->id == 38)
                                        @include('estimulos.evaluaciones.direccionServTec.transferenciaB.tblCriterio38')
                                    @endif
                                    @if ($item->id == 39)
                                        @include('estimulos.evaluaciones.direccionServTec.transferenciaB.tblCriterio39')
                                    @endif
                                    @if ($item->id == 40)
                                        @include('estimulos.evaluaciones.direccionServTec.transferenciaB.tblCriterio40')
                                    @endif
                                    @if ($item->id == 41)
                                        @include('estimulos.evaluaciones.direccionServTec.transferenciaB.tblCriterio41')
                                    @endif
                                </div>
                            @endforeach
                        </div>
