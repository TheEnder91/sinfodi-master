...
<script>
    ...
    function verTablaCriterio39(year, criterio){
        consultarDatos({
            ...
            type: 'GET',
            dataType: 'json',
            ok: function(datosServiciosCriterio39){
                var datosServiciosCriterio39 = datosServiciosCriterio39.response;
                // console.log(datosServiciosCriterio39);
                var row = "";
                for(var i = 0; i < datosServiciosCriterio39.length; i++){
                    var dataServiciosCriteri39 = datosServiciosCriterio39[i];
                    // console.log(dataServiciosCriteri39);
                    var authUser = '<?= Auth::user()->usuario ?>';
                    var permissions = '<?= Auth::user()->hasPermissionTo("estimulo-evaluaciones-servicios-transferenciaB-index") ?>';
                    // console.log(permissions);
                    if(dataServiciosCriteri39.username == authUser || permissions == 1){
                            row += "<tr>";
                            row += '<th style="font-size:12px;" scope="row" class="text-center" width="10%">' + dataServiciosCriteri39.clave + '</td>';
                            row += '<td style="font-size:12px;" width="60%">' + dataServiciosCriteri39.nombre.toUpperCase() + "</td>";
                            row += '<td style="font-size:12px;" class="text-center" width="10%">' + parseInt(dataServiciosCriteri39.puntos) + '</td>';
                            row += '<td style="font-size:12px;" class="text-center" width="10%">' + parseInt(dataServiciosCriteri39.total_puntos) + '</td>';
                            row += '<td style="font-size:12px;" class="text-center" width="10%">' + dataServiciosCriteri39.year + '</td>';
                            row += "</tr>";
                    }
                }
