...
@section('breadcrumb')
    <ol class="breadcrumb float-sm-right">
        ...
        <li class="breadcrumb-item active">Evaluación a la dirección de servicios->Transferencia de conocimiento e innovación</li>
    </ol>
@endsection

@section('content')
    @component('components.card')
        @slot('title_card', 'Evaluación a la dirección de servicios tecnologicos->Transferencia de conocimiento e innovación.')
        <div class="row">
            <div class="col-3">
                        ...
                        <div class="tab-content" id="custom-tabs-one-tabContent">
                            @foreach ($criterios as $item)
                                <div class="tab-pane fade show {{ $item->id == 15 ? 'active' : '' }}" id="objetivos-{{ $item->id }}" role="tabpanel" aria-labelledby="objetivo-{{ $item->id }}">
                                    @if ($item->id == 15)
                                        @include('estimulos.evaluaciones.direccionServTec.transferencia.tblCriterio15')
                                    @endif
                                    @if ($item->id == 16)
                                        @include('estimulos.evaluaciones.direccionServTec.transferencia.tblCriterio16')
                                    @endif
                                    @if ($item->id == 17)
                                        @include('estimulos.evaluaciones.direccionServTec.transferencia.tblCriterio17')
                                    @endif
                                    @if ($item->id == 18)
                                        @include('estimulos.evaluaciones.direccionServTec.transferencia.tblCriterio18')
                                    @endif
                                    @if ($item->id == 19)
                                        @include('estimulos.evaluaciones.direccionServTec.transferencia.tblCriterio19')
                                    @endif
                                    @if ($item->id == 20)
                                        @include('estimulos.evaluaciones.direccionServTec.transferencia.tblCriterio20')
                                    @endif
                                    @if ($item->id == 21)
                                        @include('estimulos.evaluaciones.direccionServTec.transferencia.tblCriterio21')
                                    @endif
                                    @if ($item->id == 22)
                                        @include('estimulos.evaluaciones.direccionServTec.transferencia.tblCriterio22')
                                    @endif
                                    @if ($item->id == 23)
                                        @include('estimulos.evaluaciones.direccionServTec.transferencia.tblCriterio23')
                                    @endif
                                </div>
                            @endforeach
                        </div>
