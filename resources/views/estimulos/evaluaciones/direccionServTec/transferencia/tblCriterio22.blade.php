...
@include('estimulos.evaluaciones.direccionServTec.transferencia.modalEvidenciasCriterio22')
...
<script>
    ...
    function verTablaCriterio22(year, criterio){
        consultarDatos({
            ...
            type: 'GET',
            dataType: 'json',
            ok: function(datosServiciosCriterio22){
                // console.log(datosServiciosCriterio22);
                var datosServiciosCriterio22 = datosServiciosCriterio22.response;
                var row = "";
                for(var i = 0; i < datosServiciosCriterio22.length; i++){
                    var dataServiciosCriterio22 = datosServiciosCriterio22[i];
                    // console.log(dataServiciosCriterio22);
                    var authUser = '<?= Auth::user()->usuario ?>';
                    var permissions = '<?= Auth::user()->hasPermissionTo("estimulo-evaluaciones-servicios-transferencia-index") ?>';
                    // console.log(permissions);
                    if(dataServiciosCriterio22.username == authUser || permissions == 1){
                        row += "<tr>";
                        row += '<th scope="row" class="text-center" width="10%" style="font-size:12px;">' + dataServiciosCriterio22.clave + '</td>';
                        row += '<td width="40%" style="font-size:12px;">' + dataServiciosCriterio22.nombre.toUpperCase() + "</td>";
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + parseInt(dataServiciosCriterio22.puntos) + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + parseInt(dataServiciosCriterio22.total_puntos) + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + dataServiciosCriterio22.year + '</td>';
                        // if(permissions == 1){
                            row += '<td class="text-center" width="10%" style="font-size:12px;"><a href="javascript:verEvidenciasCriterio22(' + dataServiciosCriterio22.year + ', ' + dataServiciosCriterio22.clave + ', ' + 22 +')"><i class="fa fa-edit"></i></a></td>';
                        // }
                        row += "</tr>";
                    }
                }
