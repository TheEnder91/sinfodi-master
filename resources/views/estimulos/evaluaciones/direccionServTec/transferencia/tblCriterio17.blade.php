...
@include('estimulos.evaluaciones.direccionServTec.transferencia.modalEvidenciasCriterio17')
...
<script>
    ...
    function verTablaCriterio17(year, criterio){
        consultarDatos({
            ...
            type: 'GET',
            dataType: 'json',
            ok: function(datosServiciosCriterio17){
                // console.log(datosServiciosCriterio17);
                var datosServiciosCriterio17 = datosServiciosCriterio17.response;
                var row = "";
                for(var i = 0; i < datosServiciosCriterio17.length; i++){
                    var dataServiciosCriterio17 = datosServiciosCriterio17[i];
                    // console.log(dataServiciosCriterio17);
                    var authUser = '<?= Auth::user()->usuario ?>';
                    var permissions = '<?= Auth::user()->hasPermissionTo("estimulo-evaluaciones-servicios-transferencia-index") ?>';
                    // console.log(permissions);
                    if(dataServiciosCriterio17.username == authUser || permissions == 1){
                        row += "<tr>";
                        row += '<th scope="row" class="text-center" width="10%" style="font-size:12px;">' + dataServiciosCriterio17.clave + '</td>';
                        row += '<td width="40%" style="font-size:12px;">' + dataServiciosCriterio17.nombre + "</td>";
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + dataServiciosCriterio17.puntos + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + dataServiciosCriterio17.total_puntos + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + dataServiciosCriterio17.year + '</td>';
                        // if(permissions == 1){
                            row += '<td class="text-center" width="10%" style="font-size:12px;"><a href="javascript:verEvidenciasCriterio17(' + dataServiciosCriterio17.year + ', ' + dataServiciosCriterio17.clave + ', ' + 17 +')"><i class="fa fa-edit"></i></a></td>';
                        // }
                        row += "</tr>";
                    }
                }
