<div class="table-responsive" width = "100%" id="table_refresh">
    <table id="tblCriterio20" class="table table-bordered table-striped" style="font-size:13px;">
        <caption style="font-size:13px;">Derecho de autor otorgado (Valor del punto: 30).</caption>
        <thead>
...
@include('estimulos.evaluaciones.direccionServTec.transferencia.modalEvidenciasCriterio20')
...
<script>
    ...
    function verTablaCriterio20(year, criterio){
        consultarDatos({
            ...
            type: 'GET',
            dataType: 'json',
            ok: function(datosServiciosCriterio20){
                // console.log(datosServiciosCriterio20);
                var datosServiciosCriterio20 = datosServiciosCriterio20.response;
                var row = "";
                for(var i = 0; i < datosServiciosCriterio20.length; i++){
                    var dataServiciosCriterio20 = datosServiciosCriterio20[i];
                    // console.log(dataServiciosCriterio20);
                    var authUser = '<?= Auth::user()->usuario ?>';
                    var permissions = '<?= Auth::user()->hasPermissionTo("estimulo-evaluaciones-servicios-transferencia-index") ?>';
                    // console.log(permissions);
                    if(dataServiciosCriterio20.username == authUser || permissions == 1){
                        row += "<tr>";
                        row += '<th scope="row" class="text-center" width="10%" style="font-size:12px;">' + dataServiciosCriterio20.clave + '</td>';
                        row += '<td width="40%" style="font-size:12px;">' + dataServiciosCriterio20.nombre + "</td>";
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + dataServiciosCriterio20.puntos + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + dataServiciosCriterio20.total_puntos + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + dataServiciosCriterio20.year + '</td>';
                        // if(permissions == 1){
                            row += '<td class="text-center" width="10%" style="font-size:12px;"><a href="javascript:verEvidenciasCriterio20(' + dataServiciosCriterio20.year + ', ' + dataServiciosCriterio20.clave + ', ' + 20 +')"><i class="fa fa-edit"></i></a></td>';
                        // }
                        row += "</tr>";
                    }
                }
