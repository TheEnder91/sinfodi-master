...
@include('estimulos.evaluaciones.direccionServTec.transferencia.modalEvidenciasCriterio19')
...
<script>
    ...
    function verTablaCriterio19(year, criterio){
        consultarDatos({
            ...
            type: 'GET',
            dataType: 'json',
            ok: function(datosServiciosCriterio19){
                // console.log(datosServiciosCriterio19);
                var datosServiciosCriterio19 = datosServiciosCriterio19.response;
                var row = "";
                for(var i = 0; i < datosServiciosCriterio19.length; i++){
                    var dataServiciosCriterio19 = datosServiciosCriterio19[i];
                    // console.log(dataServiciosCriterio19);
                    var authUser = '<?= Auth::user()->usuario ?>';
                    var permissions = '<?= Auth::user()->hasPermissionTo("estimulo-evaluaciones-servicios-transferencia-index") ?>';
                    // console.log(permissions);
                    if(dataServiciosCriterio19.username == authUser || permissions == 1){
                        row += "<tr>";
                        row += '<th scope="row" class="text-center" width="10%" style="font-size:12px;">' + dataServiciosCriterio19.clave + '</td>';
                        row += '<td width="40%" style="font-size:12px;">' + dataServiciosCriterio19.nombre + "</td>";
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + dataServiciosCriterio19.puntos + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + dataServiciosCriterio19.total_puntos + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + dataServiciosCriterio19.year + '</td>';
                        if(permissions == 1){
                            row += '<td class="text-center" width="10%" style="font-size:12px;"><a href="javascript:verEvidenciasCriterio19(' + dataServiciosCriterio19.year + ', ' + dataServiciosCriterio19.clave + ', ' + 19 +')"><i class="fa fa-edit"></i></a></td>';
                        }
                        row += "</tr>";
                    }
                }
