...
@include('estimulos.evaluaciones.direccionServTec.transferencia.modalEvidenciasCriterio18')
...
<script>
    ...
    function verTablaCriterio18(year, criterio){
        consultarDatos({
            ...
            type: 'GET',
            dataType: 'json',
            ok: function(datosServiciosCriterio18){
                // console.log(datosServiciosCriterio18);
                var datosServiciosCriterio18 = datosServiciosCriterio18.response;
                var row = "";
                for(var i = 0; i < datosServiciosCriterio18.length; i++){
                    var dataServiciosCriterio18 = datosServiciosCriterio18[i];
                    // console.log(dataServiciosCriterio18);
                    var authUser = '<?= Auth::user()->usuario ?>';
                    var permissions = '<?= Auth::user()->hasPermissionTo("estimulo-evaluaciones-servicios-transferencia-index") ?>';
                    // console.log(permissions);
                    if(dataServiciosCriterio18.username == authUser || permissions == 1){
                        row += "<tr>";
                        row += '<th scope="row" class="text-center" width="10%" style="font-size:12px;">' + dataServiciosCriterio18.clave + '</td>';
                        row += '<td width="40%" style="font-size:12px;">' + dataServiciosCriterio18.nombre + "</td>";
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + dataServiciosCriterio18.puntos + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + dataServiciosCriterio18.total_puntos + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + dataServiciosCriterio18.year + '</td>';
                        if(permissions == 1){
                            row += '<td class="text-center" width="10%" style="font-size:12px;"><a href="javascript:verEvidenciasCriterio18(' + dataServiciosCriterio18.year + ', ' + dataServiciosCriterio18.clave + ', ' + 18 +')"><i class="fa fa-edit"></i></a></td>';
                        }
                        row += "</tr>";
                    }
                }
