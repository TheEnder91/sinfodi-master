...
@include('estimulos.evaluaciones.direccionServTec.transferencia.modalEvidenciasCriterio16')
...
<script>
    ...
    function verTablaCriterio16(year, criterio){
        consultarDatos({
            ...
            type: 'GET',
            dataType: 'json',
            ok: function(datosServiciosCriterio16){
                // console.log(datosServiciosCriterio16);
                var datosServiciosCriterio16 = datosServiciosCriterio16.response;
                var row = "";
                for(var i = 0; i < datosServiciosCriterio16.length; i++){
                    var dataServiciosCriterio16 = datosServiciosCriterio16[i];
                    // console.log(dataServiciosCriterio16);
                    var authUser = '<?= Auth::user()->usuario ?>';
                    var permissions = '<?= Auth::user()->hasPermissionTo("estimulo-evaluaciones-servicios-transferencia-index") ?>';
                    // console.log(permissions);
                    if(dataServiciosCriterio16.username == authUser || permissions == 1){
                        row += "<tr>";
                        row += '<th scope="row" class="text-center" width="10%" style="font-size:12px;">' + dataServiciosCriterio16.clave + '</td>';
                        row += '<td width="40%" style="font-size:12px;">' + dataServiciosCriterio16.nombre + "</td>";
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + dataServiciosCriterio16.puntos + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + dataServiciosCriterio16.total_puntos + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + dataServiciosCriterio16.year + '</td>';
                        // if(permissions == 1){
                            row += '<td class="text-center" width="10%" style="font-size:12px;"><a href="javascript:verEvidenciasCriterio16(' + dataServiciosCriterio16.year + ', ' + dataServiciosCriterio16.clave + ', ' + 16 +')"><i class="fa fa-edit"></i></a></td>';
                        // }
                        row += "</tr>";
                    }
                }
