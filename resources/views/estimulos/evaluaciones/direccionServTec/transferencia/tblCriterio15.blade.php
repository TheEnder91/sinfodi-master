...
@include('estimulos.evaluaciones.direccionServTec.transferencia.modalEvidenciasCriterio15')
...
<script>
    ...
    function verTablaCriterio15(year, criterio){
        consultarDatos({
            ...
            type: 'GET',
            dataType: 'json',
            ok: function(datosServiciosCriterio15){
                // console.log(datosServiciosCriterio15);
                var datosServiciosCriterio15 = datosServiciosCriterio15.response;
                var row = "";
                for(var i = 0; i < datosServiciosCriterio15.length; i++){
                    var dataServiciosCriterio15 = datosServiciosCriterio15[i];
                    // console.log(dataServiciosCriterio15);
                    var authUser = '<?= Auth::user()->usuario ?>';
                    var permissions = '<?= Auth::user()->hasPermissionTo("estimulo-evaluaciones-servicios-transferencia-index") ?>';
                    // console.log(permissions);
                    if(dataServiciosCriterio15.username == authUser || permissions == 1){
                        row += "<tr>";
                        row += '<th scope="row" class="text-center" width="10%" style="font-size:12px;">' + dataServiciosCriterio15.clave + '</td>';
                        row += '<td width="40%" style="font-size:12px;">' + dataServiciosCriterio15.nombre.toUpperCase() + "</td>";
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + parseInt(dataServiciosCriterio15.puntos) + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + parseInt(dataServiciosCriterio15.total_puntos) + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + dataServiciosCriterio15.year + '</td>';
                        if(permissions == 1){
                            row += '<td class="text-center" width="10%" style="font-size:12px;"><a href="javascript:verEvidenciasCriterio15(' + dataServiciosCriterio15.year + ', ' + dataServiciosCriterio15.clave + ', ' + 15 +')"><i class="fa fa-edit"></i></a></td>';
                        }
                        row += "</tr>";
                    }
                }
