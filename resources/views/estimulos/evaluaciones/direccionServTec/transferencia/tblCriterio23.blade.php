...
@include('estimulos.evaluaciones.direccionServTec.transferencia.modalEvidenciasCriterio23')
...
<script>
    ...
    function verTablaCriterio23(year, criterio){
        consultarDatos({
            ...
            type: 'GET',
            dataType: 'json',
            ok: function(datosServiciosCriterio23){
                // console.log(datosServiciosCriterio23);
                var datosServiciosCriterio23 = datosServiciosCriterio23.response;
                var row = "";
                for(var i = 0; i < datosServiciosCriterio23.length; i++){
                    var dataServiciosCriterio23 = datosServiciosCriterio23[i];
                    // console.log(dataServiciosCriterio23);
                    var authUser = '<?= Auth::user()->usuario ?>';
                    var permissions = '<?= Auth::user()->hasPermissionTo("estimulo-evaluaciones-servicios-transferencia-index") ?>';
                    // console.log(permissions);
                    if(dataServiciosCriterio23.username == authUser || permissions == 1){
                        row += "<tr>";
                        row += '<th scope="row" class="text-center" width="10%" style="font-size:12px;">' + dataServiciosCriterio23.clave + '</td>';
                        row += '<td width="40%" style="font-size:12px;">' + dataServiciosCriterio23.nombre.toUpperCase() + "</td>";
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + parseInt(dataServiciosCriterio23.puntos) + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + parseInt(dataServiciosCriterio23.total_puntos) + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + dataServiciosCriterio23.year + '</td>';
                        // if(permissions == 1){
                            row += '<td class="text-center" width="10%" style="font-size:12px;"><a href="javascript:verEvidenciasCriterio23(' + dataServiciosCriterio23.year + ', ' + dataServiciosCriterio23.clave + ', ' + 23 +')"><i class="fa fa-edit"></i></a></td>';
                        // }
                        row += "</tr>";
                    }
                }
