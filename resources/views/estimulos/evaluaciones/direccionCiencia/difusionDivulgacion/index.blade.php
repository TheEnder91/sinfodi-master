...
@section('breadcrumb')
    <ol class="breadcrumb float-sm-right">
        ...
        <li class="breadcrumb-item active">Evaluacion a la dirección de ciencia->Difusión y Divulgación</li>
    </ol>
@endsection

@section('content')
    @component('components.card')
        @slot('title_card', 'Evaluación a la dirección de ciencia->Difusión y Divulgación')
        <div class="row">
            <div class="col-3">
        ...
        <div class="table-responsive">
            <table id="tblCriterio1" class="table table-bordered table-striped">
                <caption style="font-size:13px;">Listado de evaluación Dirección de ciencia->Difusión y Divulgación y sus respectivos puntos.</caption>
                <thead>
                    <tr class="text-center">
                        ...
                        {{-- @if (Auth::user()->hasPermissionTo("estimulo-evaluaciones-ciencia-difusiondivulgacion-index")) --}}
        ...
        @include('estimulos.evaluaciones.direccionCiencia.difusionDivulgacion.modalEvidenciasCriterio1')
    @endcomponent
@endsection
...
<script>
    ...
    function obtenerCriterio1(year){
        ...
        // Codigo de prueba...
        consultarDatos({
            action: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionCiencia/DifDiv/searchDifDIv/" + año,
            type: 'GET',
            dataType: 'json',
            ok: function(datosCritero1){
                ...
                for(var i = 0; i < datosCriterio1.length; i++){
                    ...
                    $.ajax({
                        type: 'POST',
                        url: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionCiencia/DifDiv/saveDatosDifDiv",
                        data: {
    ...
    function verTablaCriterio1(año, criterio){
        consultarDatos({
            action: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionCiencia/DifDiv/datosDifDiv/" + año + "/" + criterio,
            type: 'GET',
            dataType: 'json',
    ...
    function verEvidenciasCriterio1(year, clave){
        ...
        var objetivo = 1;
        consultarDatos({
            action: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionCiencia/DifDiv/searchEvidenciasDifDiv/" + year + "/" + clave,
            type: 'GET',
            dataType: 'json',
            ok: function(dataEvidenciasCriterio1){
                $('#modalEvidenciasCriterio1').modal({backdrop: 'static', keyboard: false});
                consultarDatos({
                    action: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionCiencia/DifDiv/puntosDifDiv/" + criterio + "/" + objetivo,
                    type: 'GET',
                    dataType: 'json',
                    ok: function(puntosCriterio1){
                        ...
                        consultarDatos({
                            action: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionCiencia/DifDiv/getEvidenciasDifDiv/"+clave+"/"+year+"/"+criterio,
                            type: 'GET',
                            dataType: 'json',
                            ok: function(getEvidenciasCriterio1){
                                // console.log(getEvidenciasCriterio1.response);
                                var array = getEvidenciasCriterio1.response;
                                if(array.length > 0){
                                    var evidencias = [];
                                    var serieEvidencias = "";
    ...
    function actualizarEvidenciasCriterio1(){
        ...
        var objetivo = 1;
        consultarDatos({
            action: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionCiencia/DifDiv/obtenerEvidenciasDifDiv/" + clave + "/" + year + "/" + criterio,
            type: 'GET',
            dataType: 'json',
                    ...
                    ok: function(puntosCriterio1){
                        ...
                        if(cantidadEvidencias == 0){
                            ...
                            if(existe == 0){
                                $.ajax({
                                    type: 'POST',
                                    url: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionCiencia/DifDiv/savePuntos",
                                    data: {
                                        token: $('#txtTokenRepo').val(),
                                        clave: clave,
                                    ...
                                    success: function(data){
                                        consultarDatos({
                                            ...
                                            ok: function(getEvidenciasCriterio1){
                                                ...
                                                $.ajax({
                                                    type: 'PUT',
                                                    url: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionCiencia/DifDiv/updateDatosPuntos",
                                                    data: {
                                                        token: $('#txtTokenRepo').val(),
                                                        clave: clave,
                                                        id_criterio: criterio,
                                ...
                                $.ajax({
                                    type: 'PUT',
                                    url: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionCiencia/DifDiv/updateDatosDifDiv",
                                    data: {
                                        token: $('#txtTokenRepo').val(),
                                        clave: clave,
                                        evidencias: serieEvidencias,
