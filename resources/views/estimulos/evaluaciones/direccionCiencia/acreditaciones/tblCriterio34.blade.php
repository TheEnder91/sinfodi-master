...
@include('estimulos.evaluaciones.direccionCiencia.acreditaciones.modalEvidenciasCriterio34')

<script>
    function obtenerCriterio34(year, criterio){
        consultarDatos({
            action: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionCiencia/acreditaciones/searchAcreditaciones/" + year + "/" + criterio,
            type: 'GET',
            dataType: 'json',
            ok: function(datosCritero34 ){
                ...
                if(datosCriterio34.length > 0){
                    ...
                    for(var i = 0; i < datosCriterio34.length; i++){
                        ...
                        consultarDatos({
                            action: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionCiencia/posgrado/searchUsernamePosgrado/" + dataCriterio34.numero_personal,
                            type: 'GET',
                            dataType: 'json',
                            ok: function(datosCritero34Username){
                                ...
                                $.ajax({
                                    type: 'POST',
                                    url: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionCiencia/acreditaciones/saveDatosAcreditaciones",
                                    data: {
    ...
    function verTablaCriterio34(year, criterio){
        consultarDatos({
            ...
            type: 'GET',
            dataType: 'json',
            ok: function(datosCienciaCriterio34){
                // console.log(datosCienciaCriterio34);
                var datosCienciaCriterio34 = datosCienciaCriterio34.response;
                var row = "";
                for(var i = 0; i < datosCienciaCriterio34.length; i++){
                    var dataCienciaCriterio34 = datosCienciaCriterio34[i];
                    // console.log(dataCienciaCriterio34);
                    var authUser = '<?= Auth::user()->usuario ?>';
                    var permissions = '<?= Auth::user()->hasPermissionTo("estimulo-evaluaciones-ciencia-acreditaciones-index") ?>';
                    // console.log(permissions);
                    if(dataCienciaCriterio34.username == authUser || permissions == 1){
                        row += "<tr>";
                        row += '<th scope="row" class="text-center" width="10%" style="font-size:12px;">' + dataCienciaCriterio34.clave + '</td>';
                        row += '<td width="40%" style="font-size:12px;">' + dataCienciaCriterio34.nombre.toUpperCase() + "</td>";
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + parseInt(dataCienciaCriterio34.puntos) + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + parseInt(dataCienciaCriterio34.total_puntos) + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + dataCienciaCriterio34.year + '</td>';
                        // if(permissions == 1){
                            row += '<td class="text-center" width="10%" style="font-size:12px;"><a href="javascript:verEvidenciasCriterio34(' + dataCienciaCriterio34.year + ', ' + dataCienciaCriterio34.clave + ', ' + 34 +')"><i class="fa fa-edit"></i></a></td>';
                        // }
                        row += "</tr>";
                    }
                }
    ...
    function verEvidenciasCriterio34(year, clave){
        ...
        consultarDatos({
            ...
            ok: function(dataEvidenciasCriterio34){
                ...
                consultarDatos({
                    action: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionCiencia/acreditaciones/puntos/" + criterio + "/" + objetivo,
                    type: 'GET',
                    dataType: 'json',
                    ok: function(puntosCriterio34){
                        ...
                        consultarDatos({
                            action: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionCiencia/acreditaciones/getEvidenciasAcreditaciones/"+clave+"/"+year+"/"+criterio,
                            type: 'GET',
                            dataType: 'json',
    ...
    function actualizarEvidenciasCriterio34(){
        ...
        consultarDatos({
            action: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionCiencia/acreditaciones/obtenerEvidencias/" + clave + "/" + year + "/" + criterio,
            type: 'GET',
            dataType: 'json',
                    ...
                    ok: function(puntosCriterio34){
                        ...
                        if(cantidadEvidencias == 0){
                            ...
                            if(existe == 0){
                                $.ajax({
                                    type: 'POST',
                                    url: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionCiencia/acreditaciones/savePuntos",
                                    data: {
                                        token: $('#txtTokenRepo').val(),
                                        clave: clave,
                                    ...
                                    success: function(data){
                                        consultarDatos({
                                            ...
                                            ok: function(getEvidenciasCriterio34){
                                                ...
                                                $.ajax({
                                                    type: 'PUT',
                                                    url: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionCiencia/acreditaciones/updateDatosPuntos",
                                                    data: {
                                                        token: $('#txtTokenRepo').val(),
                                                        clave: clave,
                                                        id_criterio: criterio,
                                ...
                                $.ajax({
                                    type: 'PUT',
                                    url: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionCiencia/acreditaciones/updateDatos",
                                    data: {
                                        token: $('#txtTokenRepo').val(),
                                        clave: clave,
                                        evidencias: serieEvidencias,
