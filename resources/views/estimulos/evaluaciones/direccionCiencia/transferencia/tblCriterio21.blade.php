...
@include('estimulos.evaluaciones.direccionCiencia.transferencia.modalEvidenciasCriterio21')
...
<script>
    ...
    function verTablaCriterio21(year, criterio){
        consultarDatos({
            ...
            type: 'GET',
            dataType: 'json',
            ok: function(datosCienciaCriterio21){
                // console.log(datosCienciaCriterio21);
                var datosCienciaCriterio21 = datosCienciaCriterio21.response;
                var row = "";
                for(var i = 0; i < datosCienciaCriterio21.length; i++){
                    var dataCienciaCriterio21 = datosCienciaCriterio21[i];
                    // console.log(dataCienciaCriterio21);
                    var authUser = '<?= Auth::user()->usuario ?>';
                    var permissions = '<?= Auth::user()->hasPermissionTo("estimulo-evaluaciones-ciencia-transferencia-index") ?>';
                    // console.log(permissions);
                    if(dataCienciaCriterio21.username == authUser || permissions == 1){
                        row += "<tr>";
                        row += '<th scope="row" class="text-center" width="10%">' + dataCienciaCriterio21.clave + '</td>';
                        row += '<td width="40%">' + dataCienciaCriterio21.nombre + "</td>";
                        row += '<td class="text-center" width="10%">' + dataCienciaCriterio21.puntos + '</td>';
                        row += '<td class="text-center" width="10%">' + dataCienciaCriterio21.total_puntos + '</td>';
                        row += '<td class="text-center" width="10%">' + dataCienciaCriterio21.year + '</td>';
                        if(permissions == 1){
                            row += '<td class="text-center" width="10%"><a href="javascript:verEvidenciasCriterio21(' + dataCienciaCriterio21.year + ', ' + dataCienciaCriterio21.clave + ', ' + 21 +')"><i class="fa fa-edit"></i></a></td>';
                        }
                        row += "</tr>";
                    }
                }
    ...
    //                             }
    //                             actualizarDatosCienciaCriterio21(clave, year, 21);
    ...
    // function actualizarDatosCienciaCriterio21(clave, year, criterio){
