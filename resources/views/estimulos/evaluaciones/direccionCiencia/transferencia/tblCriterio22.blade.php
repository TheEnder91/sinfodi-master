<div class="table-responsive" width = "100%" id="table_refresh">
    <table id="tblCriterio22" class="table table-bordered table-striped" style="font-size:13px;">
        <caption style="font-size:13px;">Patente otorgada (Valor del punto: 80).</caption>
        <thead>
...
@include('estimulos.evaluaciones.direccionCiencia.transferencia.modalEvidenciasCriterio22')
...
<script>
    ...
    function verTablaCriterio22(year, criterio){
        consultarDatos({
            ...
            type: 'GET',
            dataType: 'json',
            ok: function(datosCienciaCriterio22){
                // console.log(datosCienciaCriterio22);
                var datosCienciaCriterio22 = datosCienciaCriterio22.response;
                var row = "";
                for(var i = 0; i < datosCienciaCriterio22.length; i++){
                    var dataCienciaCriterio22 = datosCienciaCriterio22[i];
                    // console.log(dataCienciaCriterio22);
                    var authUser = '<?= Auth::user()->usuario ?>';
                    var permissions = '<?= Auth::user()->hasPermissionTo("estimulo-evaluaciones-ciencia-transferencia-index") ?>';
                    // console.log(permissions);
                    if(dataCienciaCriterio22.username == authUser || permissions == 1){
                        row += "<tr>";
                        row += '<th scope="row" class="text-center" width="10%" style="font-size:12px;">' + dataCienciaCriterio22.clave + '</td>';
                        row += '<td width="40%" style="font-size:12px;">' + dataCienciaCriterio22.nombre.toUpperCase() + "</td>";
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + parseInt(dataCienciaCriterio22.puntos) + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + parseInt(dataCienciaCriterio22.total_puntos) + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + dataCienciaCriterio22.year + '</td>';
                        // if(permissions == 1){
                            row += '<td class="text-center" width="10%" style="font-size:12px;"><a href="javascript:verEvidenciasCriterio22(' + dataCienciaCriterio22.year + ', ' + dataCienciaCriterio22.clave + ', ' + 22 +')"><i class="fa fa-edit"></i></a></td>';
                        // }
                        row += "</tr>";
                    }
                }
                ...
                consultarDatos({
                    ...
                    ok: function(puntosCriterio22){
                        ...
                        for(var i = 0; i < datos.length; i++){
                            ...
                            row += '</a><br>';
                            row += '<b><input type="checkbox" class="evidenciasCriterio22" name="evidenciasCriterio22[]" id="evidenciasCriterio22'+claveData.clave+'" value="'+claveData.porcentaje / 100+'" onClick="contarEvidenciasCriterio22(this.checked, '+puntos+', this.value);"> ' + claveData.clave + tipo + ' -> ' + claveData.porcentaje + '%</b>';
                            row += '</div>';
                        }
    ...
    function contarEvidenciasCriterio22(estaChequeado, puntos, porcentaje){
        // Variables...
        var contador = 0;
        ...
        var campo_resultado = document.getElementById('txtTotalCriterio22');
        // Revisamos errores...
        try {
