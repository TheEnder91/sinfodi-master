...
@include('estimulos.evaluaciones.direccionCiencia.transferencia.modalEvidenciasCriterio15')
...
<script>
    ...
    function verTablaCriterio15(year, criterio){
        consultarDatos({
            ...
            type: 'GET',
            dataType: 'json',
            ok: function(datosCienciaCriterio15){
                // console.log(datosCienciaCriterio15);
                var datosCienciaCriterio15 = datosCienciaCriterio15.response;
                var row = "";
                for(var i = 0; i < datosCienciaCriterio15.length; i++){
                    var dataCienciaCriterio15 = datosCienciaCriterio15[i];
                    // console.log(dataCienciaCriterio15);
                    var authUser = '<?= Auth::user()->usuario ?>';
                    var permissions = '<?= Auth::user()->hasPermissionTo("estimulo-evaluaciones-ciencia-transferencia-index") ?>';
                    // console.log(permissions);
                    if(dataCienciaCriterio15.username == authUser || permissions == 1){
                        row += "<tr>";
                        row += '<th scope="row" class="text-center" width="10%">' + dataCienciaCriterio15.clave + '</td>';
                        row += '<td width="40%">' + dataCienciaCriterio15.nombre + "</td>";
                        row += '<td class="text-center" width="10%">' + dataCienciaCriterio15.puntos + '</td>';
                        row += '<td class="text-center" width="10%">' + dataCienciaCriterio15.total_puntos + '</td>';
                        row += '<td class="text-center" width="10%">' + dataCienciaCriterio15.year + '</td>';
                        if(permissions == 1){
                            row += '<td class="text-center" width="10%"><a href="javascript:verEvidenciasCriterio15(' + dataCienciaCriterio15.year + ', ' + dataCienciaCriterio15.clave + ', ' + 15 +')"><i class="fa fa-edit"></i></a></td>';
                        }
                        row += "</tr>";
                    }
                }
    ...
    //             // console.log(existe);
    //             consultarDatos({
    //                 action: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionCiencia/investigacion/puntosinvestigacion/" + id + "/" + objetivo,
    //                 type: 'GET',
    //                 dataType: 'json',
    ...
    //                             }
    //                             actualizarDatosCienciaCriterio15(clave, year, 15);
    ...
    // function actualizarDatosCienciaCriterio15(clave, year, criterio){
