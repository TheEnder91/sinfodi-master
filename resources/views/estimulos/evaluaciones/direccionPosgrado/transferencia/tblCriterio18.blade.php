<div class="table-responsive" width = "100%" id="table_refresh">
    <table id="tblCriterio18" class="table table-bordered table-striped" style="font-size:13px;">
        <caption style="font-size:13px;">Registro de diseño industrial solicitado.</caption>
        <thead>
...
@include('estimulos.evaluaciones.direccionPosgrado.transferencia.modalEvidenciasCriterio18')
...
<script>
    ...
    function verTablaCriterio18(year, criterio){
        consultarDatos({
            ...
            type: 'GET',
            dataType: 'json',
            ok: function(datosPosgradoCriterio18){
                // console.log(datosPosgradoCriterio18);
                var datosPosgradoCriterio18 = datosPosgradoCriterio18.response;
                var row = "";
                for(var i = 0; i < datosPosgradoCriterio18.length; i++){
                    var dataPosgradoCriterio18 = datosPosgradoCriterio18[i];
                    // console.log(dataPosgradoCriterio18);
                    var authUser = '<?= Auth::user()->usuario ?>';
                    var permissions = '<?= Auth::user()->hasPermissionTo("estimulo-evaluaciones-posgrado-transferencia-index") ?>';
                    // console.log(permissions);
                    if(dataPosgradoCriterio18.username == authUser || permissions == 1){
                        row += "<tr>";
                        row += '<th scope="row" class="text-center" width="10%" style="font-size:12px;">' + dataPosgradoCriterio18.clave + '</td>';
                        row += '<td width="40%" style="font-size:12px;">' + dataPosgradoCriterio18.nombre.toUpperCase() + "</td>";
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + parseInt(dataPosgradoCriterio18.puntos) + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + parseInt(dataPosgradoCriterio18.total_puntos) + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + dataPosgradoCriterio18.year + '</td>';
                        // if(permissions == 1){
                            row += '<td class="text-center" width="10%" style="font-size:12px;"><a href="javascript:verEvidenciasCriterio18(' + dataPosgradoCriterio18.year + ', ' + dataPosgradoCriterio18.clave + ', ' + 18 +')"><i class="fa fa-edit"></i></a></td>';
                        // }
                        row += "</tr>";
                    }
                }
