...
@include('estimulos.evaluaciones.direccionPosgrado.transferencia.modalEvidenciasCriterio16')
...
<script>
    ...
    function verTablaCriterio16(year, criterio){
        consultarDatos({
            ...
            type: 'GET',
            dataType: 'json',
            ok: function(datosPosgradoCriterio16){
                // console.log(datosPosgradoCriterio16);
                var datosPosgradoCriterio16 = datosPosgradoCriterio16.response;
                var row = "";
                for(var i = 0; i < datosPosgradoCriterio16.length; i++){
                    var dataPosgradoCriterio16 = datosPosgradoCriterio16[i];
                    // console.log(dataPosgradoCriterio16);
                    var authUser = '<?= Auth::user()->usuario ?>';
                    var permissions = '<?= Auth::user()->hasPermissionTo("estimulo-evaluaciones-posgrado-transferencia-index") ?>';
                    // console.log(permissions);
                    if(dataPosgradoCriterio16.username == authUser || permissions == 1){
                        row += "<tr>";
                        row += '<th scope="row" class="text-center" width="10%" style="font-size:12px;">' + dataPosgradoCriterio16.clave + '</td>';
                        row += '<td width="40%" style="font-size:12px;">' + dataPosgradoCriterio16.nombre.toUpperCase() + "</td>";
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + parseInt(dataPosgradoCriterio16.puntos) + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + parseInt(dataPosgradoCriterio16.total_puntos) + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + dataPosgradoCriterio16.year + '</td>';
                        if(permissions == 1){
                            row += '<td class="text-center" width="10%" style="font-size:12px;"><a href="javascript:verEvidenciasCriterio16(' + dataPosgradoCriterio16.year + ', ' + dataPosgradoCriterio16.clave + ', ' + 16 +')"><i class="fa fa-edit"></i></a></td>';
                        }
                        row += "</tr>";
                    }
                }
                ...
                consultarDatos({
                    ...
                    ok: function(puntosCriterio16){
                        ...
                        for(var i = 0; i < datos.length; i++){
                            ...
                            row += '</a><br>';
                            row += '<b><input type="checkbox" class="evidenciasCriterio16" name="evidenciasCriterio16[]" id="evidenciasCriterio16'+claveData.clave+'" value="'+claveData.clave+'" onClick="contarEvidenciasCriterio16('+puntos+');"> ' + claveData.clave + tipo + '</b>';
                            row += '</div>';
                        }
    ...
    function contarEvidenciasCriterio16(puntos){
