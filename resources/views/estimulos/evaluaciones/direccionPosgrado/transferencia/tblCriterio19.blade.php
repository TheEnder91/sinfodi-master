...
@include('estimulos.evaluaciones.direccionPosgrado.transferencia.modalEvidenciasCriterio19')
...
<script>
    ...
    function verTablaCriterio19(year, criterio){
        consultarDatos({
            ...
            type: 'GET',
            dataType: 'json',
            ok: function(datosPosgradoCriterio19){
                // console.log(datosPosgradoCriterio19);
                var datosPosgradoCriterio19 = datosPosgradoCriterio19.response;
                var row = "";
                for(var i = 0; i < datosPosgradoCriterio19.length; i++){
                    var dataPosgradoCriterio19 = datosPosgradoCriterio19[i];
                    // console.log(dataPosgradoCriterio19);
                    var authUser = '<?= Auth::user()->usuario ?>';
                    var permissions = '<?= Auth::user()->hasPermissionTo("estimulo-evaluaciones-posgrado-transferencia-index") ?>';
                    // console.log(permissions);
                    if(dataPosgradoCriterio19.username == authUser || permissions == 1){
                        row += "<tr>";
                        row += '<th scope="row" class="text-center" width="10%" style="font-size:12px;">' + dataPosgradoCriterio19.clave + '</td>';
                        row += '<td width="40%" style="font-size:12px;">' + dataPosgradoCriterio19.nombre.toUpperCase() + "</td>";
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + parseInt(dataPosgradoCriterio19.puntos) + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + parseInt(dataPosgradoCriterio19.total_puntos) + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + dataPosgradoCriterio19.year + '</td>';
                        if(permissions == 1){
                            row += '<td class="text-center" width="10%" style="font-size:12px;"><a href="javascript:verEvidenciasCriterio19(' + dataPosgradoCriterio19.year + ', ' + dataPosgradoCriterio19.clave + ', ' + 19 +')"><i class="fa fa-edit"></i></a></td>';
                        }
                        row += "</tr>";
                    }
                }
