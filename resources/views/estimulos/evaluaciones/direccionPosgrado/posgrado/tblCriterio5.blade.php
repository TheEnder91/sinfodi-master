<div class="table-responsive">
    <table id="tblCriterio5" class="table table-bordered table-striped" style="font-size:13px;">
        <caption style="font-size:13px;">Alumno del programa de doctorado del CIDETEQ graduado entre 43 y 48 meses (Valor del punto: 60).</caption>
        <thead>
...
<script>
            ...
            ok: function(datosPosgradoCriterio5){
                ...
                for(var i = 0; i < datosPosgradoCriterio5.length; i++){
                    ...
                    if(dataPosgradoCriterio5.username == authUser || permissions == 1){
                        ...
                        // if(permissions == 1){
                            row += '<td class="text-center" width="10%" style="font-size:12px;"><a href="javascript:verEvidenciasCriterio5(' + dataPosgradoCriterio5.year + ', ' + dataPosgradoCriterio5.clave + ', ' + 5 +')"><i class="fa fa-edit"></i></a></td>';
                        // }
                        row += "</tr>";
                    }
                }
    ...
    function verEvidenciasCriterio5(year, clave, criterio){
        ...
        consultarDatos({
            action: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionPosgrado/posgrado/searchEvidenciasPosgrado/" + year + "/" + clave + "/" + criterio,
            type: 'GET',
            dataType: 'json',
            ok: function(dataEvidenciasCriterio5){
                ...
                consultarDatos({
                    action: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionPosgrado/posgrado/puntosPosgrado/" + criterio + "/" + objetivo,
                    type: 'GET',
                    dataType: 'json',
                    ok: function(puntosCriterio5){
                        ...
                        consultarDatos({
                            action: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionPosgrado/posgrado/getEvidenciasPosgrado/" + clave + "/" + year + "/" + criterio,
                            type: 'GET',
                            dataType: 'json',
            ...
            ok: function(searchEvidenciasCriterio5){
                ...
                if(cantidadEvidencias == 0){
                    ...
                    if(existe == 0){
                        $.ajax({
                            type: 'POST',
                            url: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionPosgrado/posgrado/savePuntos",
                            data: {
                                token: $('#txtTokenRepo').val(),
                                clave: clave,
                            ...
                            success: function(data){
                                ...
                                consultarDatos({
                                    ...
                                    ok: function(getEvidenciasCriterio5){
                                        ...
                                        $.ajax({
                                            type: 'PUT',
                                            url: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionPosgrado/posgrado/updateDatosPuntos",
                                            data: {
                                                token: $('#txtTokenRepo').val(),
                                                clave: clave,
                                                id_criterio: criterio,
