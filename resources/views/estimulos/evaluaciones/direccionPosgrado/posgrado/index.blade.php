...
@section('breadcrumb')
    <ol class="breadcrumb float-sm-right">
        ...
        <li class="breadcrumb-item active">Evaluaciones a la dirección de posgrado->Posgrado</li>
    </ol>
@endsection
                        ...
                        <div class="tab-content" id="custom-tabs-one-tabContent">
                            @foreach ($criterios as $item)
                                <div class="tab-pane fade show {{ $item->id == 2 ? 'active' : '' }}" id="objetivos-{{ $item->id }}" role="tabpanel" aria-labelledby="objetivo-{{ $item->id }}">
                                    @if ($item->id == 2)
                                        @include('estimulos.evaluaciones.direccionPosgrado.posgrado.tblCriterio2')
                                    @endif
                                    @if ($item->id == 3)
                                        @include('estimulos.evaluaciones.direccionPosgrado.posgrado.tblCriterio3')
                                    @endif
                                    @if ($item->id == 4)
                                        @include('estimulos.evaluaciones.direccionPosgrado.posgrado.tblCriterio4')
                                    @endif
                                    @if ($item->id == 5)
                                        @include('estimulos.evaluaciones.direccionPosgrado.posgrado.tblCriterio5')
                                    @endif
                                </div>
                            @endforeach
                        </div>
