...
@section('scripts')
    <script>
        ...
        function verTablaCriterio36(){
            ...
            var criterio = 36;
            consultarDatos({
            action: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionPosgrado/investigacionB/datosInvestigacionB/" + year + "/" + criterio,
            type: 'GET',
            dataType: 'json',
            ok: function(datosPosgradoCriterio36){
                var datosPosgradoCriterio36 = datosPosgradoCriterio36.response;
                // console.log(datosPosgradoCriterio36);
                var row = "";
                for(var i = 0; i < datosPosgradoCriterio36.length; i++){
                    var dataPosgradoCriteri40 = datosPosgradoCriterio36[i];
                    // console.log(dataPosgradoCriteri40);
                    var authUser = '<?= Auth::user()->usuario ?>';
                    var permissions = '<?= Auth::user()->hasPermissionTo("estimulo-evaluaciones-posgrado-investigacionB-index") ?>';
