<div class="table-responsive" width = "100%">
    <table id="tblCriterio7" class="table table-bordered table-striped" style="font-size:13px;">
        <caption style="font-size:13px;">Publicación de Artículos en revistas de circulación internacional con arbitraje, factor de impacto entre 2.0 y 4.0.</caption>
        <thead>
...
@include('estimulos.evaluaciones.direccionPosgrado.investigacion.modalEvidenciasCriterio7')
...
<script>
    ...
    function verTablaCriterio7(year, criterio){
        consultarDatos({
            ...
            type: 'GET',
            dataType: 'json',
            ok: function(datosPosgradoCriterio7){
                // console.log(datosPosgradoCriterio7);
                var datosPosgradoCriterio7 = datosPosgradoCriterio7.response;
                var row = "";
                for(var i = 0; i < datosPosgradoCriterio7.length; i++){
                    var dataPosgradoCriterio7 = datosPosgradoCriterio7[i];
                    // console.log(dataPosgradoCriterio7);
                    var authUser = '<?= Auth::user()->usuario ?>';
                    var permissions = '<?= Auth::user()->hasPermissionTo("estimulo-evaluaciones-posgrado-investigacion-index") ?>';
                    // console.log(permissions);
                    if(dataPosgradoCriterio7.username == authUser || permissions == 1){
                        row += "<tr>";
                        row += '<th scope="row" class="text-center" width="10%" style="font-size:12px;">' + dataPosgradoCriterio7.clave + '</td>';
                        row += '<td width="40%" style="font-size:12px;">' + dataPosgradoCriterio7.nombre.toUpperCase() + "</td>";
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + parseInt(dataPosgradoCriterio7.puntos) + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + parseInt(dataPosgradoCriterio7.total_puntos) + '</td>';
                        row += '<td class="text-center" width="10%" style="font-size:12px;">' + dataPosgradoCriterio7.year + '</td>';
                        if(permissions == 1){
                            row += '<td class="text-center" width="10%" style="font-size:12px;"><a href="javascript:verEvidenciasCriterio7(' + dataPosgradoCriterio7.year + ', ' + dataPosgradoCriterio7.clave + ', ' + 7 +')"><i class="fa fa-edit"></i></a></td>';
                        }
                        row += "</tr>";
                    }
                }
