...
@section('breadcrumb')
    <ol class="breadcrumb float-sm-right">
        ...
        <li class="breadcrumb-item active">Evaluaciones a la dirección de administración->Investigación científica.</li>
    </ol>
@endsection
                        ...
                        <div class="tab-content" id="custom-tabs-one-tabContent">
                            @foreach ($criterios as $item)
                                <div class="tab-pane fade show {{ $item->id == 6 ? 'active' : '' }}" id="objetivos-{{ $item->id }}" role="tabpanel" aria-labelledby="objetivo-{{ $item->id }}">
                                    @if ($item->id == 6)
                                        @include('estimulos.evaluaciones.direccionPosgrado.investigacion.tblCriterio6')
                                    @endif
                                    @if ($item->id == 7)
                                        @include('estimulos.evaluaciones.direccionPosgrado.investigacion.tblCriterio7')
                                    @endif
                                    @if ($item->id == 8)
                                        @include('estimulos.evaluaciones.direccionPosgrado.investigacion.tblCriterio8')
                                    @endif
                                    @if ($item->id == 9)
                                        @include('estimulos.evaluaciones.direccionPosgrado.investigacion.tblCriterio9')
                                    @endif
                                    @if ($item->id == 10)
                                        @include('estimulos.evaluaciones.direccionPosgrado.investigacion.tblCriterio10')
                                    @endif
                                    @if ($item->id == 11)
                                        @include('estimulos.evaluaciones.direccionPosgrado.investigacion.tblCriterio11')
                                    @endif
                                    @if ($item->id == 12)
                                        @include('estimulos.evaluaciones.direccionPosgrado.investigacion.tblCriterio12')
                                    @endif
                                    @if ($item->id == 13)
                                        @include('estimulos.evaluaciones.direccionPosgrado.investigacion.tblCriterio13')
                                    @endif
                                </div>
                            @endforeach
                        </div>
