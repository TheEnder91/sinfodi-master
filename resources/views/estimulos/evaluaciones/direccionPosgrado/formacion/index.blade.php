...
@section('breadcrumb')
    <ol class="breadcrumb float-sm-right">
        ...
        <li class="breadcrumb-item active">Evaluaciones a la dirección de posgrado->Formación de recursos humanos</li>
    </ol>
@endsection

@section('content')
    @component('components.card')
        @slot('title_card', 'Evaluaciones a la dirección de posgrado->Formación de recursos humanos.')
        <div class="row">
            <div class="col-3">
                        ...
                        <div class="tab-content" id="custom-tabs-one-tabContent">
                            @foreach ($criterios as $item)
                                <div class="tab-pane fade show {{ $item->id == 24 ? 'active' : '' }}" id="objetivos-{{ $item->id }}" role="tabpanel" aria-labelledby="objetivo-{{ $item->id }}">
                                    @if ($item->id == 24)
                                        @include('estimulos.evaluaciones.direccionPosgrado.formacion.tblCriterio24')
                                    @endif
                                    @if ($item->id == 25)
                                        @include('estimulos.evaluaciones.direccionPosgrado.formacion.tblCriterio25')
                                    @endif
                                    @if ($item->id == 26)
                                        @include('estimulos.evaluaciones.direccionPosgrado.formacion.tblCriterio26')
                                    @endif
                                    @if ($item->id == 27)
                                        @include('estimulos.evaluaciones.direccionPosgrado.formacion.tblCriterio27')
                                    @endif
                                    @if ($item->id == 28)
                                        @include('estimulos.evaluaciones.direccionPosgrado.formacion.tblCriterio28')
                                    @endif
                                    @if ($item->id == 29)
                                        @include('estimulos.evaluaciones.direccionPosgrado.formacion.tblCriterio29')
                                    @endif
                                    @if ($item->id == 30)
                                        @include('estimulos.evaluaciones.direccionPosgrado.formacion.tblCriterio30')
                                    @endif
                                    @if ($item->id == 31)
                                        @include('estimulos.evaluaciones.direccionPosgrado.formacion.tblCriterio31')
                                    @endif
                                </div>
                            @endforeach
                        </div>
