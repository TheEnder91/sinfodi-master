...
@section('breadcrumb')
    <ol class="breadcrumb float-sm-right">
        ...
        <li class="breadcrumb-item active">Evaluaciones a la dirección de posgrado->Sostenibilidad economica B</li>
    </ol>
@endsection

@section('content')
    @component('components.card')
        @slot('title_card', 'Evaluaciones a la dirección de posgrado->Sostenibilidad economica B.')
        <div class="row">
            <div class="col-3">
...
@section('scripts')
    <script>
        ...
        function obtenerCriterio37(year){
            ...
            var criterio = 37;
            consultarDatos({
                action: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionPosgrado/sostenibilidadB/searchSostenibilidadB/" + año,
                type: 'GET',
                dataType: 'json',
                ok: function(datosCritero37){
                    ...
                    if(datosCriterio37.length > 0){
                        ...
                        // console.log(arr2);
                        consultarDatos({
                            action: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionPosgrado/sostenibilidadB/puntos/" + criterio + "/" + id_objetivo,
                            type: 'GET',
                            dataType: 'json',
                            ok: function(puntosCriterio37){
                                ...
                                for(var i = 0; i < arr2.length; i++){
                                    ...
                                    $.ajax({
                                        type: 'POST',
                                        url: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionPosgrado/sostenibilidadB/saveDatos",
                                        data: {
        ...
        function verTablaCriterio37(year, criterio){
            consultarDatos({
                action: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionPosgrado/sostenibilidadB/datosSostenibilidadB/" + year + "/" + criterio,
                type: 'GET',
                dataType: 'json',
                ok: function(datosGeneralCriterio37){
                    ...
                    for(var i = 0; i < datosGeneralCriterio37.length; i++){
                        ...
                        var permissions = '<?= Auth::user()->hasPermissionTo("estimulo-evaluaciones-posgrado-sostentabilidadB-index") ?>';
        ...
        function verDetalleCriterio37(year, clave){
            ...
            consultarDatos({
                action: "{{ config('app.url') }}/estimulos/evaluaciones/DireccionPosgrado/sostenibilidadB/getDatosRemanente/" + clave + "/" + year,
                type: 'GET',
                dataType: 'json',
