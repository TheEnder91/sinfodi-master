...
<script>
    ...
    function verTablaCriterio38(year, criterio){
        consultarDatos({
            ...
            type: 'GET',
            dataType: 'json',
            ok: function(datosPosgradoCriterio38){
                var datosPosgradoCriterio38 = datosPosgradoCriterio38.response;
                // console.log(datosPosgradoCriterio38);
                var row = "";
                for(var i = 0; i < datosPosgradoCriterio38.length; i++){
                    var dataPosgradoCriteri38 = datosPosgradoCriterio38[i];
                    // console.log(dataPosgradoCriteri38);
                    var authUser = '<?= Auth::user()->usuario ?>';
                    var permissions = '<?= Auth::user()->hasPermissionTo("estimulo-evaluaciones-posgrado-transferenciaB-index") ?>';
                    // console.log(permissions);
                    if(dataPosgradoCriteri38.username == authUser || permissions == 1){
                            row += "<tr>";
                            row += '<th style="font-size:12px;" scope="row" class="text-center" width="10%">' + dataPosgradoCriteri38.clave + '</td>';
                            row += '<td style="font-size:12px;" width="60%">' + dataPosgradoCriteri38.nombre.toUpperCase() + "</td>";
                            row += '<td style="font-size:12px;" class="text-center" width="10%">' + parseInt(dataPosgradoCriteri38.puntos) + '</td>';
                            row += '<td style="font-size:12px;" class="text-center" width="10%">' + parseInt(dataPosgradoCriteri38.total_puntos) + '</td>';
                            row += '<td style="font-size:12px;" class="text-center" width="10%">' + dataPosgradoCriteri38.year + '</td>';
                            row += "</tr>";
                    }
                }
