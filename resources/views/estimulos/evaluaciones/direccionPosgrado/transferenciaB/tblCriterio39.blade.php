...
<script>
    ...
    function verTablaCriterio39(year, criterio){
        consultarDatos({
            ...
            type: 'GET',
            dataType: 'json',
            ok: function(datosPosgradoCriterio39){
                var datosPosgradoCriterio39 = datosPosgradoCriterio39.response;
                // console.log(datosPosgradoCriterio39);
                var row = "";
                for(var i = 0; i < datosPosgradoCriterio39.length; i++){
                    var dataPosgradoCriteri39 = datosPosgradoCriterio39[i];
                    // console.log(dataPosgradoCriteri39);
                    var authUser = '<?= Auth::user()->usuario ?>';
                    var permissions = '<?= Auth::user()->hasPermissionTo("estimulo-evaluaciones-posgrado-transferenciaB-index") ?>';
                    // console.log(permissions);
                    if(dataPosgradoCriteri39.username == authUser || permissions == 1){
                            row += "<tr>";
                            row += '<th style="font-size:12px;" scope="row" class="text-center" width="10%">' + dataPosgradoCriteri39.clave + '</td>';
                            row += '<td style="font-size:12px;" width="60%">' + dataPosgradoCriteri39.nombre.toUpperCase() + "</td>";
                            row += '<td style="font-size:12px;" class="text-center" width="10%">' + parseInt(dataPosgradoCriteri39.puntos) + '</td>';
                            row += '<td style="font-size:12px;" class="text-center" width="10%">' + parseInt(dataPosgradoCriteri39.total_puntos) + '</td>';
                            row += '<td style="font-size:12px;" class="text-center" width="10%">' + dataPosgradoCriteri39.year + '</td>';
                            row += "</tr>";
                    }
                }
